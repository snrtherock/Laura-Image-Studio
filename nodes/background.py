...
class BackgroundRemover:
    ...
    def remove_background(self, image, rmbg_model="RMBG-2.0", threshold=0.5, invert_mask=False):
        ...
        if RMBGNode is not None:
            ...
            print("[Laura Studio] RMBG not installed. Using simple elliptical mask fallback.")
...
class BackgroundReplacer:
    ...
    def replace_background(self, foreground, mask, background,
                          edge_feather=4, blend_mode="normal"):
        import torch.nn.functional as F
...
class BackgroundGenerator:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "prompt": ("STRING", {"multiline": True, "default": "professional studio background, gradient, soft lights"}),
                "seed": ("INT", {"default": 0, "min": 0, "max": 0xffffffffffffffff}),
                "steps": ("INT", {"default": 25, "min": 1, "max": 100}),
                "cfg": ("FLOAT", {"default": 5.0, "min": 0.0, "max": 20.0}),
            },
            "optional": {
                "negative_prompt": ("STRING", {"multiline": True, "default": "person, human, face, deformed, blurry, artifacts"}),
            }
        }
    ...
    def generate_background(self, model, clip, vae, width, height, prompt,
                           seed, steps, cfg,
...
class ProLighting:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE",),
                "mask": ("MASK",),
                "lighting_type": (["rim_light", "soft_box", "hair_light",
                                   "butterfly", "split", "rembrandt"],),
    ...
    def apply_lighting(self, image, mask, lighting_type, intensity,
                      color="#FFFFFF", direction="right"):
        # Parse color
        c = color.lstrip('#')
        if len(c) < 6:
            c = c + "0" * (6 - len(c))
